...
for (yr in years) { # yr = years[1]
  tif <- glue("{output_dir}/{dataset_code}_IDW_{species_common}_{yr}.tif")
...
yr <- 2015
r <- rast(glue("{output_dir}/{dataset_code}_IDW_{species_common}_{yr}.tif"))
r
```
...
ggplot() +
  ...
  coord_fixed() +
  labs(title = glue("{dataset_code} {species_common}, {yr}"))
```
